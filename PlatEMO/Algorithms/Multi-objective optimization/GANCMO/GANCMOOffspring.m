function [O1,O2,Guided] = GANCMOOffspring(P,P1,P2,F1,F2,n1,n2,q,Archive,seed,generation)
% Preserve the independent GA/DE streams, parent order and evaluation order.
    ...
    for k = 1:2
        ...
        if ~isempty(local)
            Y = GANCMOBoundaryStep((proposal(local,:)-P.lower)./span, ...
        ...
        if m>0
            ...
            assert(~any(ismember(proposal,q,'rows')),'GANCMO:DirectQuery','A query must never be evaluated directly.');
        end
...
function idx = parentIndices(Pop,Fitness,count)
    n = numel(Pop);
    assert(count==0 || n>=3,'GANCMO:DEPopulationTooSmall','DE needs at least three parents.');
