function FrontNo = EADMMConstraintNDSort(PopObj,PopCon,nSort)
% Nondominated sorting under EADMM's binary constraint-dominance rule.

    violationCount = sum(~(PopCon<=0),2);
    N              = size(PopObj,1);
    FrontNo        = inf(1,N);
    ...
    for p = 1 : N-1
        for q = p+1 : N
            if violationCount(p) < violationCount(q)
                relation = 1;
            elseif violationCount(q) < violationCount(p)
                relation = -1;
            else
                relation = ParetoRelation(PopObj(p,:),PopObj(q,:));
            end
            if relation == 1
