function [Population,Fitness] = ...
        PairGuideSelection(Population,N,isConstrained)
%PAIRGUIDESELECTION CCMO/SPEA2 survival for both populations.
...
%--------------------------------------------------------------------------
    ...
    if isConstrained
        Fitness = PairGuideFitness(Population.objs,Population.cons);
    else
        Fitness = PairGuideFitness(Population.objs);
    end
    Next = Fitness < 1;
