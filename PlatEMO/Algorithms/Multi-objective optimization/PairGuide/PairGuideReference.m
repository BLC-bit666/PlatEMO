function [Ref,Scale,Yn] = PairGuideReference(Y,W,Scale)
%PAIRGUIDEREFERENCE Assign objectives under one reusable scale.
%   REF = PAIRGUIDEREFERENCE(Y,W) min-max normalizes Y and assigns
%   each row to the reference vector with maximum cosine similarity.
%   REF = PAIRGUIDEREFERENCE(Y,W,SCALE) reuses SCALE.minimum and
...
%--------------------------------------------------------------------------
    ...
    if size(W,2) ~= objectiveCount
        error('PairGuide:ReferenceDimensionMismatch', ...
    ...
    if nargin < 3 || isempty(Scale)
        ...
        if ~isstruct(Scale) || ~isfield(Scale,'minimum') || ...
                ~isfield(Scale,'span')
            error('PairGuide:InvalidReferenceScale', ...
    ...
    if isempty(Y)
        ...
        return;
    elseif isempty(W)
        error('PairGuide:EmptyReferenceVectors', ...
