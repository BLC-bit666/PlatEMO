...
唯一主线为 **双种群协同进化 + 真实边界配对 + 条件 WGAN 引导 + coverage α=0.04 资源控制**。直接运行 `PairGuide` 即使用该方案，无候选机制、消融模式或实验参数切换。

## 完整流程

1. 初始化 P1/P2 和参考向量。P1 考虑约束，P2 忽略约束；用真实评价结果构建 F/I 配对存档。
2. 每代两群分别生成 25% GA、75% DE 子代。P1 的最后 m 个 DE 槽接收上一代筛选的 m 个 q，按决策距离贪心匹配。
3. 对引导槽，先作中点桥接并执行原 DE 交叉、修复与变异；再由 q 定位最近真实 F/I 线段，将子代收缩到该线段中点周围。只评价最终子代，q 不直接评价或进入种群。
4. P1/P2 各自从自身与两群子代中作环境选择；用选择前完整联合种群更新真实端点存档，保留最多500对，每方向一个 active 对。
5. 用前沿身份差异 S 与参考方向覆盖率 C 更新单一状态 z，得到下代名额 Q 与停止比例 tau。若下一完整批次越过截止 FE，永久停用 CGAN，继续普通双种群搜索至总预算耗尽。
6. 未停止时，用 active 端点按原门槛首训/续训 WGAN-GP；训练侧别均衡、方向均衡。已有模型且仍有有效对时生成500个候选。
7. 盒内候选按到真实 F 的距离选取，另给未覆盖且盒外方向最多 `floor(.2*Q)` 个探索名额，每方向最多1点。缺额先退回盒内，再由普通DE补齐；筛选结果留给下一代。
...
这些是既定设计常数，不是新增的公开调参接口；不得把整体算法称为无参数。具体端点变化度与归一化见 `PairGuideModel.m`，执行契约见根目录 `Agent.md`。

## 文件职责

| 文件 | 职责 |
| --- | --- |
| `PairGuide.m` | 唯一入口与代际流程，负责永久停用 |
| `PairGuideControl.m` | 单一资源状态、名额与截止FE |
| `PairGuideArchive.m` | 真实端点收紧、重配对、容量竞争和训练数据 |
| `PairGuideModel.m` | 固定条件 WGAN-GP 的训练、续训、采样与数值失败回退 |
| `PairGuideCandidates.m` | 500个条件查询及盒内/盒外筛选 |
| `PairGuideOffspring.m` | 原随机流下的GA/DE及q的一一消费 |
| `PairGuideBoundaryStep.m` | q定位的真实F/I局部收缩 |
| `PairGuideFitness.m` | SPEA2强度与密度适应度 |
| `PairGuideSelection.m` | 双种群约束/无约束环境选择 |
| `PairGuideReference.m` | 一致目标尺度下的参考方向分配 |
...
```matlab
platemo('algorithm',@PairGuide,'problem',@LIRCMOP5_BC, ...
