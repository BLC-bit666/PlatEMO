...
%   Additional oracle calls are cached and billed separately after search.
    ...
    if isfile(cacheFile)
        ...
        if isfield(old.Report,'rendererVersion') && old.Report.rendererVersion==8 && ...
    ...
    for k=1:numel(indices)
        ...
        S=struct('targetFE',targets(k),'observationFE',P.observationFE, ...
            ...
            'nativeMode',nativeMode,'singleMode',singleMode,'useSideCondition',true,'sides',zeros(0,1), ...
        ...
        if indices(k)>1
            ...
            if ~isempty(q)
                ...
                if singleMode
                    ...
                    if isfield(Q,'useSideCondition')
                        S.useSideCondition=Q.useSideCondition;
                        if ~S.useSideCondition; S.labelAccuracy=NaN; end
                    end
                    frame=E.population{Q.generation+1}.referenceScale;
                    if isfield(Q,'conditionScale'); frame=Q.conditionScale; end
    ...
    Report=struct('rendererVersion',8,'rendererSourceHash',rendererHash, ...
...
function renderFigure(Problem,problem,S,label,file,paired,focused)
    ...
    for pane=1:panes
        ...
        elseif pane==2
            ...
            if S.singleMode && S.useSideCondition
                delete(h1);
            ...
            if paired
                ...
                elseif S.singleMode
                    title(ax,'Single-point CGAN | side condition disabled');
                end
            else
    ...
    if S.singleMode
        ...
        if ~S.useSideCondition
            trainingNote=sprintf('Last training FE %g | %g pairs | %g G updates | no side condition | direction error %.1f deg', ...
                S.trainingFE,S.trainingPairs,S.trainingUpdates,S.directionError);
        end
    end
