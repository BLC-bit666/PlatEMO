function BranchSummary = run_CBS_RegionWGAN_GP_boundary_populated_branch( ...
        rootOutDir,workerCount,problemNames,N,D,maxFE,runIds,Overrides)
%RUN_CBS_REGIONWGAN_GP_BOUNDARY_POPULATED_BRANCH Run exact-mainline query branch.
%   This branch keeps the promoted C WGAN-GP mainline unchanged except for
%   queryMode=boundary_populated. The branch tests whether sampling only
%   BMem-populated reference regions makes generated points closer to the
%   observed boundary cloud.

    repoRoot = locateRepoRoot();
    addpath(genpath(repoRoot));
    if nargin < 1 || isempty(rootOutDir)
        rootOutDir = fullfile(repoRoot,'Data','CBS_RegionGAN_compare', ...
            ['boundary_populated_mainline_', ...
    ...
    if branchComplete(outDir,numel(problemNames),numel(runIds), ...
            numel(stageTargets),drawFigures)
        Row.status = "skipped_complete";
        Row = fillCounts(Row,outDir);
    ...
    BranchSummary = writeSummary(Row,rootOutDir);
    fprintf('boundary-populated branch written to: %s\n',rootOutDir);
end
...
function Branch = makeBranch()
    Branch = struct( ...
        'name',"C_query_boundary_populated", ...
        'queryMode',"boundary_populated", ...
        'conditionMode',"region");
end
...
function yes = branchComplete(outDir,problemCount,runCount,stageCount, ...
    ...
    yes = isfile(runFile) && isfile(stageFile) && isfile(eventFile) && ...
        ...
        hasQueryMode(stageFile,"boundary_populated") && ...
        hasQueryMode(eventFile,"boundary_populated");
...
function Row = fillCounts(Row,outDir)
    ...
    Row.has_boundary_populated_query = hasQueryMode( ...
        fullfile(outDir,'stage_snapshots_all.csv'), ...
        "boundary_populated") && hasQueryMode( ...
        fullfile(outDir,'event_summary_all.csv'), ...
        "boundary_populated");
end
...
function Row = emptyBranchRow()
    Row = struct( ...
        ...
        'has_boundary_populated_query',false, ...
        'error_message',"");
end
