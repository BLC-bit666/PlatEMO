...
%   the default "de" path is byte-identical (fixed-seed regression test).
    ...
    nGenZero = false(size(arms));
    for i = 1 : numel(arms)
        modes(i) = "ga_de_half"; bls(i) = "on"; gen(i) = "wgan";
        switch arms(i)
            case "S1";  modes(i) = "imtcmo_de";  bls(i) = "off";
            case "S2";  bls(i) = "off";
            case "S2B"  % mainline switches
            case "A1";  modes(i) = "de";         bls(i) = "off";
            case "A0";  nGenZero(i) = true;      % no CGAN, budget to DE/GA
            case "GN";  gen(i) = "copynoise";    % trivial sampler
            otherwise
                error('CBSRegionGAN:BadTriageArm', ...
                    'Arms must be from A1, S1, S2, S2B, A0, and GN.');
        end
    end
    ...
    if workerCount == 1
        for task = 1 : height(Tasks)
            ...
            rows(task) = runTask(arms(a),modes(a),bls(a),gen(a), ...
                nGenZero(a),Tasks.problem(task),Tasks.seed(task), ...
        ...
        parfor task = 1 : height(Tasks)
            a = taskArm(task);
            rows(task) = runTask(arms(a),modes(a),bls(a),gen(a), ...
                nGenZero(a),taskProblem(task),taskSeed(task), ...
                outDir,N,maxFE,Options.resume);
...
function Row = runTask(armId,mode,blsState,genMode,nGenZero, ...
...
%RUNTASK Execute or reuse one arm/problem/seed task.
    ...
    if resume && isfile(Row.task_file)
        try
            ...
            if ~isfield(Candidate,'gen'); Candidate.gen = "wgan"; end
            if ~isfield(Candidate,'nGenZero'); Candidate.nGenZero = 0; end
            if string(Candidate.status) == "ok" && ...
                    string(Candidate.gen) == Row.gen && ...
                    double(Candidate.nGenZero) == Row.nGenZero && ...
    ...
    try
        ...
        args = {'save',0,'outputFcn',@quietOutput, ...
            'operatorMode',char(mode),'boundarySearch',char(blsState), ...
            'generatorMode',char(genMode)};
        if nGenZero
            Defaults = CBS_RegionWGAN_GP.mainlineDefaults();
            args = [args,{'parameter',{0,Defaults.zDim, ...
                Defaults.ganIter,Defaults.ganMiniBatch, ...
                Defaults.nCritic,Defaults.minGANTrainCount, ...
                Defaults.sampleSigma}}];
        end
...
function Row = emptyRunRow()
    Row = struct( ...
        'arm',"",'mode',"",'bls',"off",'gen',"wgan",'nGenZero',0, ...
        'problem',"",'seed',NaN,'N',NaN,'D',NaN, ...
