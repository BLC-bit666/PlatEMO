    ...
    methods(Access = protected)
        ...
        function runRegionGAN(Algorithm,Problem,ganKind,Config)
            ...
            DatasetOptions = struct( ...
                'conditionMode',regionConditionModeFromControl( ...
                    optionalConfig(Config,'conditionMode',"region")));
            ...
            GAN  = [];
            gen  = 0;
            TrainTriggerState = RunRegionGAN_RC('inittraintriggerstate');
            ...
            while Algorithm.NotTerminated(Population1)
                ...
                Diag.condition_mode = DatasetOptions.conditionMode;
                ...
                inspectTrain = ~isempty(BMem) && nGen > 0 && ...
                    (periodicTrain || regionTrainTriggerEnabled( ...
                    EventGANOptions));
                if inspectTrain
                    ...
                    if isstruct(DatasetInfo) && ...
                            isfield(DatasetInfo,'condition_mode')
                        Diag.condition_mode = string( ...
                            DatasetInfo.condition_mode);
                    end
                    Diag.train_count = size(TrainX,1);
                    Diag.query_count = regionQueryPoolCount( ...
                        QueryMode,QueryC,DatasetInfo,W);
                    triggerRefCount = regionTriggerRefCount(DatasetInfo);
                    [TrainNow,TriggerInfo] = RunRegionGAN_RC( ...
                        'traintrigger',EventGANOptions,TrainTriggerState, ...
                        gen,Diag.train_count,triggerRefCount,periodicTrain);
                    EventGANOptions = RunRegionGAN_RC('resolveganoptions', ...
                        GANOptions,Problem.FE,regionProblemMaxFE(Problem), ...
                        TriggerInfo);
                    Diag = copyGANOptionInfo(Diag,EventGANOptions);
                    Diag = copyTrainTriggerInfo(Diag,TriggerInfo);
                    if TrainNow && size(TrainX,1) >= max(minBoundaryLength, ...
                        ...
                        TrainTriggerState = RunRegionGAN_RC( ...
                            'updatetraintriggerstate',TrainTriggerState, ...
                            TriggerInfo);
                        ...
                        if ~isempty(RawDec)
                            ...
                            Diag = copyGeneratedBoundaryDiag(Diag, ...
                                BoundaryDiag);
                            WidthDiag = regionFixmdWidthDiagnostics( ...
...
function Options = applyRegionGANExperimentOptions(Options)
    ...
    if isfield(Control,'trainTriggerMode') && ...
            ~isempty(Control.trainTriggerMode)
        Options.trainTriggerMode = lower(strtrim(string( ...
            Control.trainTriggerMode)));
    end
    if isfield(Control,'trainTriggerDelta') && ...
            ~isempty(Control.trainTriggerDelta)
        Options.trainTriggerDelta = double(Control.trainTriggerDelta);
    end
...
function Diag = copyGANTrainInfo(Diag,GAN)
    ...
    if isstruct(GAN) && isfield(GAN,'last_sample_info') && ...
        ...
        if isfield(GAN.last_sample_info,'prescreen_raw_score')
            Diag.prescreen_raw_score = double( ...
                GAN.last_sample_info.prescreen_raw_score(:));
        end
        if isfield(GAN.last_sample_info,'prescreen_raw_selected')
            Diag.prescreen_raw_selected = logical( ...
                GAN.last_sample_info.prescreen_raw_selected(:));
        end
        if isfield(GAN.last_sample_info,'prescreen_raw_condition_index')
            Diag.prescreen_raw_condition_index = double( ...
                GAN.last_sample_info.prescreen_raw_condition_index(:));
        end
    end
end
...
function Diag = copyGANOptionInfo(Diag,Options)
    ...
    Diag.train_trigger_mode = getOptionString(Options, ...
        'trainTriggerMode',Diag.train_trigger_mode);
    Diag.train_trigger_delta = getOptionNumeric(Options, ...
        'trainTriggerDelta',Diag.train_trigger_delta);
end
...
function [SampleC,Counts,SampleRefs,DiagQueryC,DiagQueryRefs] = ...
    ...
    [SampleC,Counts,SampleRefs,DiagQueryC,DiagQueryRefs] = ...
        RunRegionGAN_RC('regionquerysamples',QueryMode,QueryC, ...
        DatasetInfo,W,queryPerCondition,nGen);
end

function count = regionQueryPoolCount(QueryMode,QueryC,DatasetInfo,W)
    count = RunRegionGAN_RC('regionquerypoolcount',QueryMode,QueryC, ...
        DatasetInfo,W);
end
...
function mode = regionConditionModeFromControl(defaultMode)
    mode = string(defaultMode);
    Control = regionGANExperimentControl();
    if ~isempty(Control) && isfield(Control,'conditionMode') && ...
    ...
    switch mode
        ...
        case {"region_rho","rho","radial","region_radial"}
            mode = "region_rho";
        otherwise
            error('CBSRegionGAN:BadConditionMode', ...
...
function Diag = copyTrainTriggerInfo(Diag,TriggerInfo)
    fields = fieldnames(TriggerInfo);
    for i = 1 : numel(fields)
        name = fields{i};
        value = TriggerInfo.(name);
        if isnumeric(value) || islogical(value)
            Diag.(name) = double(value);
        elseif isstring(value) || ischar(value)
            Diag.(name) = string(value);
        end
    end
end

function yes = regionTrainTriggerEnabled(Options)
    mode = getOptionString(Options,'trainTriggerMode',"off");
    yes = ~ismember(mode,["","off","none","periodic","fixed"]);
end

function count = regionTriggerRefCount(DatasetInfo)
    count = 0;
    if isstruct(DatasetInfo) && isfield(DatasetInfo,'queryRegions') && ...
            ~isempty(DatasetInfo.queryRegions)
        refs = round(double(DatasetInfo.queryRegions(:)));
    elseif isstruct(DatasetInfo) && isfield(DatasetInfo,'trainRef') && ...
            ~isempty(DatasetInfo.trainRef)
        refs = round(double(DatasetInfo.trainRef(:)));
    else
        refs = zeros(0,1);
    end
...
function Metrics = regionFixmdWidthDiagnostics(DatasetInfo,OffspringG, ...
        SampleRefs,BMem,Problem)
    Metrics = emptyFixmdWidthMetrics();
...
function Metrics = emptyFixmdWidthMetrics()
    Metrics = struct( ...
        'train_width50',NaN, ...
        'train_width90',NaN, ...
        'train_width_count',0, ...
...
function Info = emptyRegionDatasetInfo(M)
    Info = struct( ...
        ...
        'trainConditionScalar',zeros(0,0), ...
        'trainRef',zeros(0,1), ...
        'queryConditions',zeros(0,0), ...
        'queryConditionScalar',zeros(0,0), ...
        'queryRegions',zeros(0,1), ...
        'queryObjs',zeros(0,M), ...
        'allQueryConditions',zeros(0,0), ...
        'allQueryConditionScalar',zeros(0,0), ...
...
function Snapshot = makeRegionStageSnapshot(Observer,Algorithm,Problem, ...
    ...
    Snapshot.train_trigger_mode = string(Diag.train_trigger_mode);
    Snapshot.trigger_reason = string(Diag.trigger_reason);
    Snapshot.trigger_train_now = double(Diag.trigger_train_now);
    Snapshot.trigger_train_count_delta = ...
        double(Diag.trigger_train_count_delta);
