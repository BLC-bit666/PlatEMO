...
%TEST_CBS_PAIR_GUIDE Final shared-conversation contract, including real autodiff.
    ...
    outside=T.keepIdx(~T.insideArchiveBox(T.keepIdx));
    assert(T.keptInsideCount>=16 && numel(outside)<=4 && ...
        all(T.requestedUncovered(outside)) && numel(unique(refs(~T.insideArchiveBox(T.keepIdx))))==numel(outside), ...
        'Outside exploration must respect the cap and use distinct uncovered requests.');
    F.matchedPairIds = 0;
