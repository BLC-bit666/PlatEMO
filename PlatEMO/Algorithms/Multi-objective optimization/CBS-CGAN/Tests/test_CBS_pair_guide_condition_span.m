...
assert(all([A.trainingSigma B.trainingSigma C.trainingSigma D.trainingSigma]==.1));
assert(sum([A.updates B.updates C.updates D.updates])==8 && ...
    sum([A.criticUpdates B.criticUpdates C.criticUpdates D.criticUpdates])==8);
O.sampleSigma=.1;
rng(20260908,'twister'); [X0,Info0]=PairBoundaryWGAN_RC('sample',NoSide,[W,zeros(100,1)],P,O);
rng(20260908,'twister'); [X1,Info1]=PairBoundaryWGAN_RC('sample',NoSide,[W,ones(100,1)],P,O);
assert(isequal(X0,X1) && isequal(Info0.z,Info1.z) && std(Info0.z(:))>0);
assert(all(Info0.conditions(:,end)==0) && all(Info1.conditions(:,end)==0));
assert(~Info1.useSideCondition && isequal(Info1.conditionScale.span,Data.referenceScale.span));
