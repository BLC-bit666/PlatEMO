...
%   4) three probes {0.25,0.5,0.75} for boundary localization,
%   5) one local mechanism: infeasible shrink,
%   6) one migration rule: move a new feasible point into P_C only if it
%      improves the current sector champion.
% runtimeOptions expose only the fix3 validation switches:
%   DisableInfeasibleShrink / ProbeLambdas / UseDirectMix.
...
function Params = ResolvePRBCCMOParameters(ParameterCell)
    ...
    if ~isempty(UnsupportedIndex)
        error('PRBCCMO:UnsupportedParameter', ...
            'Unsupported positional parameter at index %d. PRBCCMO-Core only accepts the core 5 parameters.', ...
            UnsupportedIndex);
    end
        ...
        if strcmpi(Field,'runtimeOptions')
            ...
            if ~isempty(FieldIndex)
                ...
                error('PRBCCMO:UnsupportedParameterField', ...
                    'Unsupported PRBCCMO-Core parameter ''%s''.',Field);
            end
        end
    end
end
    ...
    for i = 1 : numel(Fields)
        if ~isfield(Defaults,Fields{i})
            error('PRBCCMO:UnsupportedRuntimeOption', ...
                'Unsupported PRBCCMO-Core runtime option ''%s''.',Fields{i});
        end
...
function Options = NormalizeRuntimeOptions(Options,~)
    ...
    Options.UseDirectMix = logical(Options.UseDirectMix) || logical(Options.DirectMix);
    Options.DirectMix = Options.UseDirectMix;
    Options.ProbeLambdas = NormalizeProbeLambdas( ...
        Options.ProbeLambdas,[0.25,0.50,0.75]);
end
...
function [OffspringC,OffspringU] = GenerateRegularOffspring( ...
    Problem,PopulationC,PopulationU,RuntimeOptions)
    UseDirectMix = SafeRuntimeOption(RuntimeOptions,'UseDirectMix',false);
    if UseDirectMix
        MixedPool = [PopulationC.decs;PopulationU.decs];
        OffspringC = Problem.Evaluation( ...
            OperatorDECurrentRand1(Problem,PopulationC.decs,MixedPool));
        OffspringU = Problem.Evaluation( ...
            OperatorDECurrentRand1(Problem,PopulationU.decs,MixedPool));
        return;
    end
...
function [MigrationPool,BoundaryBatch] = ExecuteBoundaryCore( ...
    ...
    for i = 1 : numel(Order)
        ...
        BoundaryBatch = AppendBoundaryBatch(BoundaryBatch,ProbeSolutions,true(numel(ProbeSolutions),1));
...
function Lambdas = ResolveProbeLambda(RuntimeOptions)
    DefaultLambdas = [0.25,0.50,0.75];
    if nargin < 1
        RuntimeOptions = struct();
    end
    Lambdas = SafeRuntimeOption(RuntimeOptions,'ProbeLambdas',DefaultLambdas);
    Lambdas = NormalizeProbeLambdas(Lambdas,DefaultLambdas);
end
...
function [BestIndex,BestScore] = SelectBestProbe(Score,Lambda)
    if isempty(Score)
        BestIndex = 1;
        BestScore = NaN;
        return;
    end
    OrderKey = [Score(:),abs(Lambda(:)-0.5),Lambda(:)];
    [BestIndex,~] = minrows(OrderKey);
