    ...
    methods
        function main(Algorithm,Problem)
            ...
            [Model,MLPDiag] = UpdateBoundaryModelWithDiagnostics( ...
                ...
                SectorNeighbors,size(W,1),Problem.M);
            ...
            while Algorithm.NotTerminated(PopulationC)
                ...
                Observer = LogGenerationDiagnostics( ...
                    Observer,Generation,Problem,W,SectorNeighbors,PopulationC,PopulationU, ...
...
function [Model,Diag] = UpdateBoundaryModelWithDiagnostics( ...
    ...
    Diag = InitMLPDiag( ...
        Model,TrainArchive,Gap,Tick,minPos,minNeg,SectorNeighbors,K,M);
    if ~Diag.can_train
        return;
    end
...
function B = SelectTopKPerSector(Candidates,Meta,BridgeMask,kappa)
    ...
    for s = unique(Meta.sector(Meta.sector > 0))'
        ...
        Sel = ReserveBridgeGrowthCandidate(Sel,idx,Meta,kappa);
...
function Sel = ReserveBridgeGrowthCandidate(Sel,idx,Meta,kappa)
    GrowthIdx = idx(Meta.bridgeGrowth(idx));
    if isempty(GrowthIdx)
        return;
    end

    GrowthKey = BuildBridgeGrowthKey(Meta,GrowthIdx);
    [~,ord] = sortrows(GrowthKey,1:size(GrowthKey,2));
    Pick = GrowthIdx(ord(1));
    if ismember(Pick,Sel)
        return;
    end
    if numel(Sel) < min(kappa,numel(idx))
        Sel(end+1,1) = Pick; %#ok<AGROW>
        return;
    end
...
function Key = BuildBridgeGrowthKey(Meta,idx)
    idx = idx(:);
    if isempty(idx)
        Key = zeros(0,4);
        return;
...
function [Offspring,Diag] = GenerateBoundaryOffspringModelDriven( ...
    ...
    while size(ProbeDecAll,1) < Budget && Attempt < MaxAttempt
        ...
        PairDist  = ComputeBoundaryPairDistance(Anchor,Helper);
        BetaPred  = EstimatePredictedCrossBeta(Beta,Prob);
        ...
        Diag.events = [Diag.events; { ...
            Generation,FE,Attempt,s,HelperInfo.type,double(all(Anchor.cons<=0,2)),HelperInfo.feasible, ...
            HelperMeta.sector,HelperMeta.trusted, ...
            AnchorMeta.trusted,AnchorMeta.prob,AnchorMeta.margin,AnchorMeta.oppSupport, ...
            AnchorMeta.objScore,AnchorMeta.score, ...
            HelperMeta.prob,HelperMeta.margin,HelperMeta.oppSupport, ...
            HelperMeta.objScore,HelperMeta.score, ...
            Beta(best),BetaPred,PairDist,Prob(best),chosenPred,ProbeMeta.margin(best),ProbeMeta.oppSupport(best), ...
            ProbeMeta.oppDist(best),ProbeMeta.objScore(best),Score(best),NaN,NaN,NaN}]; %#ok<AGROW>
    ...
    if isempty(ProbeDecAll)
        ...
        for i = 1 : min(numel(Feasible),size(Diag.events,1))
            Diag.events{i,end-2} = EstimateSegCrossDistProxy( ...
                Diag.events{i,22},Diag.events{i,21},Feasible(i),Diag.events{i,6},Diag.events{i,7});
            Diag.events{i,end-1} = Feasible(i);
            Diag.events{i,end} = double(Diag.events{i,25} == Feasible(i));
        end
...
function Row = ExtractBoundaryMetaRow(Meta,idx)
    ...
    Row.bridgeGrowth = Meta.bridgeGrowth(idx);
...
function Dist = ComputeBoundaryPairDistance(Anchor,Helper)
    Dist = NaN;
    if isempty(Anchor) || isempty(Helper)
        return;
    end
    Dist = norm(double(Anchor.decs) - double(Helper.decs));
end

function BetaPred = EstimatePredictedCrossBeta(Beta,Prob)
    BetaPred = NaN;
    if isempty(Beta) || isempty(Prob)
        return;
    end
    ...
    for i = 1 : numel(Beta)-1
        ...
        if d1*d2 < 0 || d2 == 0
            t = (0.5-Prob(i))/max(Prob(i+1)-Prob(i),eps);
            BetaPred = Beta(i) + t*(Beta(i+1)-Beta(i));
            return;
        end
    end
end

function Dist = EstimateSegCrossDistProxy(BetaPred,BetaChosen,ChosenFeasible,AnchorFeasible,HelperFeasible)
    Dist = NaN;
    if any(isnan([BetaPred,BetaChosen,ChosenFeasible,AnchorFeasible,HelperFeasible])) || ...
       AnchorFeasible == HelperFeasible
        return;
    end

    if ChosenFeasible == AnchorFeasible
        Lower = BetaChosen;
        Upper = 1;
    elseif ChosenFeasible == HelperFeasible
        Lower = 0;
        Upper = BetaChosen;
    else
        return;
    end
    Dist = max([Lower-BetaPred,BetaPred-Upper,0]);
end
...
function [TrainArchive,Diag] = UpdateTrainArchiveWithDiagnostics( ...
    ...
    TrainArchive = UpdateTrainArchive( ...
        TrainArchive,B,PopulationC,PopulationU,RecentBoundaryOff,W, ...
        SectorNeighbors,MaxTrain,Stamp);
    Diag = CollectTrainArchiveDiagnostics(TrainArchive,SectorNeighbors,size(W,1));
end

function Diag = CollectTrainArchiveDiagnostics(TrainArchive,SectorNeighbors,K)
    %#ok<INUSD>
    PairStats = SummarizeTrainAnchorPairs(TrainArchive);
    Diag = struct( ...
        ...
        'core_size',size(TrainArchive.Dec,1), ...
        'core_sector_coverage',NaN, ...
        'trusted_sector_count',NaN, ...
        'paired_sector_count',PairStats.pair_count, ...
        'anchor_count',PairStats.anchor_count, ...
        'pair_count',PairStats.pair_count, ...
        'mean_pair_dist',PairStats.mean_pair_dist);
end

function Stats = SummarizeTrainAnchorPairs(TrainArchive)
    Stats = struct('anchor_count',0,'pair_count',0,'mean_pair_dist',NaN);
    ...
    Stats.anchor_count = numel(AnchorIdx);
    if isfield(TrainArchive,'Weight') && numel(TrainArchive.Weight) == size(TrainArchive.Dec,1)
        Stats.pair_count = sum(TrainArchive.Weight(NegIdx));
    else
        Stats.pair_count = numel(NegIdx);
    end
    if isempty(AnchorIdx) || isempty(NegIdx)
        return;
    end

    AnchorDec = TrainArchive.Dec(AnchorIdx,:);
    NegDec    = TrainArchive.Dec(NegIdx,:);
    Dist = pdist2(AnchorDec,NegDec);
    Stats.mean_pair_dist = mean(min(Dist,[],2),'omitnan');
end
...
function Diag = InitMLPDiag( ...
    Model,TrainArchive,Gap,Tick,minPos,minNeg,SectorNeighbors,K,M)
    Base = CollectTrainArchiveDiagnostics(TrainArchive,SectorNeighbors,K);
    Diag = struct( ...
        'gap',Gap, ...
        'tick',Tick, ...
        'train_size',Base.size, ...
        'pos_count',Base.pos_count, ...
        'neg_count',Base.neg_count, ...
        'src_b',Base.src_b, ...
        'src_recent_boundary',Base.src_recent_boundary, ...
        'src_rep_c',Base.src_rep_c, ...
        'src_rep_u',Base.src_rep_u, ...
        'core_size',Base.core_size, ...
        'core_sector_coverage',Base.core_sector_coverage, ...
        'trusted_sector_count',Base.trusted_sector_count, ...
        'paired_sector_count',Base.paired_sector_count, ...
        'anchor_count',Base.anchor_count, ...
        'pair_count',Base.pair_count, ...
        'mean_pair_dist',Base.mean_pair_dist, ...
        'can_train',double(CanTrainBoundaryModel( ...
...
function Stats = EvaluatePopulationMaskPredictions(Model,Population,Label,Mask)
    if isempty(Population) || isempty(Label) || isempty(Mask) || ~any(Mask)
        Stats = InitPredictionStats();
        return;
    end

    Dec = Population.decs;
    Stats = EvaluateBinaryPredictions(Model,Dec(Mask,:),Label(Mask));
end
...
function Observer = InitObserver(Algorithm,Problem,Params,ProbeBeta,MaxTrain)
    ...
    WriteCsvHeader(Observer.summary_file,{ ...
        ...
        'boundary_sector_coverage','boundary_mixed_sectors', ...
        'boundary_trusted_count','boundary_trusted_sector_coverage','boundary_bridge_growth_count', ...
        'boundary_mean_margin','boundary_mean_opp_support','boundary_mean_oppdist', ...
        'boundary_mean_score','boundary_lowmargin_count', ...
        'boundary_mean_pair_dist','boundary_lowmargin_pair_hit','boundary_seg_cross_dist', ...
        'boundary_survive_c','boundary_survive_u', ...
        'helper_real_opp_ratio','helper_skip_no_helper','helper_skip_duplicate', ...
        'seed_b_size','seed_b_feasible_ratio','seed_b_sector_coverage','seed_b_mixed_sectors', ...
        'seed_b_trusted_count','seed_b_bridge_growth_count', ...
        'seed_b_mean_pair_dist','seed_b_lowmargin_pair_hit', ...
        'b_size','b_feasible_ratio','b_sector_coverage','b_mixed_sectors', ...
        'b_trusted_count','b_trusted_sector_coverage','b_bridge_growth_count','b_trusted_lowmargin_count', ...
        ...
        'boundary_bal_acc','boundary_brier','boundary_logloss', ...
        'b_bal_acc','b_brier','b_logloss', ...
        'b_lowmargin_bal_acc','b_lowmargin_brier', ...
        'b_trusted_bal_acc','b_trusted_brier', ...
        'train_size','train_pos','train_neg', ...
        'train_src_b','train_src_recent_boundary','train_src_rep_c','train_src_rep_u', ...
        'train_core_size','train_core_sector_coverage', ...
        'train_trusted_sector_count','train_paired_sector_count','train_can_train', ...
        'train_anchor_count','train_pair_count','train_mean_pair_dist', ...
        'model_ready_before','model_trained','model_ready_after', ...
        'train_bal_acc_after','train_brier_after','train_logloss_after', ...
        'train_minus_boundary_bal_gap','train_minus_b_bal_gap'});
    WriteCsvHeader(Observer.boundary_file,{ ...
        'generation','fe','attempt','sector','helper_type','anchor_feasible','helper_feasible', ...
        'helper_sector','helper_trusted', ...
        'anchor_trusted','anchor_prob','anchor_margin','anchor_opp_support','anchor_obj_score','anchor_score', ...
        'helper_prob','helper_margin','helper_opp_support','helper_obj_score','helper_score', ...
        'beta','beta_pred_cross','pair_dist','chosen_prob','chosen_pred_label','chosen_margin','chosen_opp_support','chosen_oppdist', ...
        'chosen_obj_score','chosen_score','seg_cross_dist','chosen_feasible','chosen_correct'});
    WriteCsvHeader(Observer.archive_file,{ ...
        'generation','fe','member_index','sector','feasible','trusted','bridge_growth', ...
        'prob','pred_label','correct', ...
        'margin','obj_score','opp_support','opp_dist','score'});
    WriteCsvHeader(Observer.mlp_file,{ ...
        'tick','gap','can_train','trusted_sector_count','paired_sector_count', ...
        'core_size','core_sector_coverage','anchor_count','pair_count','mean_pair_dist','trained','warm_start', ...
...
function Observer = LogMLPEvent(Observer,Diag)
    AppendCsvRows(Observer.mlp_file,{ ...
        Diag.tick,Diag.gap,Diag.can_train,Diag.trusted_sector_count,Diag.paired_sector_count, ...
        Diag.core_size,Diag.core_sector_coverage,Diag.anchor_count,Diag.pair_count,Diag.mean_pair_dist,Diag.trained,Diag.warm_start, ...
...
function Observer = LogGenerationDiagnostics( ...
    Observer,Generation,Problem,W,SectorNeighbors,PopulationC,PopulationU,OffspringC,OffspringU, ...
    ...
    BoundaryMeta = BuildBoundaryMeta( ...
        BoundaryOff,W,Model,PopulationC,PopulationU,TrainArchive,SectorNeighbors);
    ...
    ArchiveLowPred = EvaluatePopulationMaskPredictions( ...
        Model,B,ArchiveMeta.feasible,ArchiveMeta.margin <= 0.10);
    ArchiveTrustedPred = EvaluatePopulationMaskPredictions( ...
        Model,B,ArchiveMeta.feasible,ArchiveMeta.trusted(:) > 0);
    ...
    BoundarySegCrossDist  = MeanOrNaN(ExtractBoundaryEventNumericColumn(BoundaryDiag.events,31));
    ...
    AppendCsvRows(Observer.summary_file,{ ...
        ...
        BoundaryStats.sector_coverage,BoundaryStats.mixed_sectors, ...
        BoundaryStats.trusted_count,BoundaryStats.trusted_sector_coverage,BoundaryStats.bridge_growth_count, ...
        BoundaryStats.mean_margin,BoundaryStats.mean_opp_support,BoundaryStats.mean_oppdist, ...
        BoundaryStats.mean_score,BoundaryStats.lowmargin_count, ...
        BoundaryStats.mean_pair_dist,BoundaryStats.lowmargin_pair_hit,BoundarySegCrossDist, ...
        BoundarySurvival.selected_c,BoundarySurvival.selected_u, ...
        HelperReal,BoundaryDiag.skipped_no_helper,BoundaryDiag.skipped_duplicate, ...
        SeedStats.size,SeedStats.feasible_ratio,SeedStats.sector_coverage,SeedStats.mixed_sectors, ...
        SeedStats.trusted_count,SeedStats.bridge_growth_count, ...
        ...
        ArchiveStats.trusted_count,ArchiveStats.trusted_sector_coverage,ArchiveStats.bridge_growth_count,ArchiveStats.trusted_lowmargin_count, ...
        ...
        ArchiveStats.mean_pair_dist,ArchiveStats.lowmargin_pair_hit, ...
        BoundaryPred.bal_acc,BoundaryPred.brier,BoundaryPred.logloss, ...
        ArchivePred.bal_acc,ArchivePred.brier,ArchivePred.logloss, ...
        ArchiveLowPred.bal_acc,ArchiveLowPred.brier, ...
        ArchiveTrustedPred.bal_acc,ArchiveTrustedPred.brier, ...
        ...
        TrainDiag.core_size,TrainDiag.core_sector_coverage, ...
        TrainDiag.trusted_sector_count,TrainDiag.paired_sector_count,MLPDiag.can_train, ...
        TrainDiag.anchor_count,TrainDiag.pair_count,TrainDiag.mean_pair_dist, ...
        MLPDiag.model_ready_before,MLPDiag.trained,MLPDiag.model_ready_after, ...
        MLPDiag.stats_after.bal_acc,MLPDiag.stats_after.brier,MLPDiag.stats_after.logloss, ...
        TrainMinusBoundaryGap,TrainMinusArchiveGap});
...
function Observer = LogArchiveMembers(Observer,Generation,FE,B,Meta)
    ...
    Rows = cell(numel(B),15);
    for i = 1 : numel(B)
        predLabel = double(Meta.prob(i) >= 0.5);
        Rows(i,:) = { ...
            Generation,FE,i,Meta.sector(i),Meta.feasible(i),Meta.trusted(i),Meta.bridgeGrowth(i),Meta.prob(i), ...
...
function Stats = SummarizeArchive(B,Meta)
    ...
    Stats.bridge_growth_count = sum(Meta.bridgeGrowth > 0);
    ...
    Stats.mean_pair_dist = Stats.mean_oppdist;
    ...
    if any(LowMask)
        Stats.lowmargin_pair_hit = SafeDivide(sum(isfinite(Meta.oppDist(LowMask))),sum(LowMask));
