...
% Range-finding tuner for PRBCCMO_t using IGD only.
    ...
    AllRows = repmat(emptyRawRow(),0,1);
    for c = 1 : height(ConfigTable)
        ...
        RawResults = struct2table(AllRows);
...
function ConfigTable = makeConfigTable()
    Baseline = table( ...
        1,"baseline",20,20,0.01,3.0,0.05,10,20, ...
        'VariableNames',configColumnNames());

    rng(20260511,'twister');
    Samples = makeLatinHypercube(24,7);
    ...
    Gstart = roundToStep(scaleLinear(Samples(:,7),20,250),1);
...
function Row = runOneTask(rootDir,Config,problemName,runId,N,maxFE)
    ...
    try
        ...
        Algorithm = PRBCCMO_t( ...
            ...
            'run',double(runId), ...
            'outputFcn',@(varargin)[], ...
            'parameter',{Row.hidden,Row.epoch,Row.lr,Row.betaB,Row.etaB,Row.Tretrain,Row.Gstart,0});
...
function Summary = summarizeConfigs(RawResults,ConfigTable)
    ...
    for c = 1 : height(ConfigTable)
        ...
        Summary.mean_igd(c) = meanOrNaN(Data.igd(OK));
        Summary.median_igd(c) = medianOrNaN(Data.igd(OK));
        Summary.mean_runtime(c) = meanOrNaN(Data.runtime(Data.status == "ok"));
    end
    ...
    for p = 1 : numel(Problems)
        for r = 1 : numel(Runs)
            ...
            for i = 1 : numel(Config)
                ...
                Accum{c}(end+1,1) = Rank(i); %#ok<AGROW>
            end
        end
    end
    for c = 1 : numel(ConfigIds)
        MeanRanks(c) = meanOrNaN(Accum{c});
    end
end
...
function Value = medianOrNaN(Value)
    Value = double(Value(:));
    Value = Value(isfinite(Value));
    if isempty(Value)
        Value = NaN;
