...
%TEST_CBS_PLATEMO_COMPLIANCE Verify both public PlatEMO contracts.
    ...
    assert(isequal(pairNames, ...
        ["rawGuideCount","zDim","ganUpdates","ganMiniBatch", ...
        "nCritic","minGANTrainCount","sampleSigma"]));
    ...
    assert(PairDefaults.pairArchiveCapacity == 500 && PairDefaults.pairArchivePerRef == 1);
    assert(PairDefaults.selectionPool == "ccmo" && PairDefaults.archiveFrontDepth == 1);
    ...
    for a = 1 : numel(Entries)
        [decs,objs,cons] = platemo( ...
            ...
            'outputFcn',@(varargin)[]);
        expected = 10;
        if strcmp(func2str(Entries{a}),'PairGuide'); expected = 1; end
        assert(size(decs,1) == expected && size(decs,2) == 5);
        assert(size(objs,1) == expected && size(cons,1) == expected);
    end
    addCBSPaths(repoRoot);
