...
%   is a separate, explicitly costed offline audit.
    ...
    for k = 1:numel(files)
        ...
        for j=1:numel(E.queries)
            ...
            inBand=Q.nativeInBand==1; % NaN means unavailable for unpaired output.
            native=native+nnz(inBand);
            ...
            nativeCertified=nativeCertified+nnz(small & inBand);
            pending=pending+Q.keptCount;
            if isfield(Q,'correction')
                correction=[correction;Q.correction(isfinite(Q.correction))]; %#ok<AGROW>
            end
        end
        if E.schema=="PairGuide-single-v3" && R.Record.mode=="cgan"
            native=NaN; coarse=NaN; certified=NaN; nativeCertified=NaN;
        end
        refCount=size(E.W,1);
