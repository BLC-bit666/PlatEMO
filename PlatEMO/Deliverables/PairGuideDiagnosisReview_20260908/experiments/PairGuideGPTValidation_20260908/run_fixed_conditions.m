...
Schedule=readmatrix(fullfile(folder,'batch_schedules',sprintf('%s_seed%02d.csv',name,task.seed)));
assert(isequal(size(Schedule),[2000,batch]) && all(Schedule>=1 & Schedule<=numel(keep),'all'));
