...
%   Results use a NEW directory. Frozen PairGuideBall results remain evidence.
    ...
    Protocol = struct('schema',"PairGuide-single-v3",'options',Options, ...
...
function runTask(Task,O,sourceHash)
    ...
    if isfield(O,'trainingOptions') && ~isempty(fieldnames(O.trainingOptions))
        Algorithm.configurePairGuideTrainingExperiment(O.trainingOptions);
    end
    ...
    Record = struct('schema',"PairGuide-single-v3",'algorithm',"PairGuide", ...
