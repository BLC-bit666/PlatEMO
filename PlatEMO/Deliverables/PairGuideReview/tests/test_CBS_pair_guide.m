...
%TEST_CBS_PAIR_GUIDE Verify the PairGuide mechanism contract.
    ...
    assert(Snapshot.arm == 7 && ...
        ...
        Snapshot.nCritic == 5 && ...
        Snapshot.trainingSigma == 1 && Snapshot.sampleSigma == 0.3 && ...
    ...
    % Re-observing unchanged endpoints does not fake a new update FE.
    ...
    % A legal historical xf remains active after leaving the current P1,
    % and a feasible Union/feedback row outside the new P1 cannot replace it.
    LifecycleW = [1 1];
    lifecycleXf = [0.10 0.10];
    lifecycleXi = [0.45 0.55];
    lifecycleYf = [0 1];
    lifecycleYi = [0.10 0.90];
    Historical = makeArchive(lifecycleXf,lifecycleXi, ...
        lifecycleYf,lifecycleYi,1);
    Historical.lastFE = 123;
    currentX = [0.95 0.95];
    currentY = [0 0.94];
    replacementX = [0.40 0.50];
    replacementY = [0.05 0.95];
    Current = SOLUTION(currentX,currentY,0);
    Replacement = SOLUTION(replacementX,replacementY,0);
    VerticalInfeasible = SOLUTION([1 1],[0 1],1);
    Feedback = struct('childDecs',replacementX, ...
        'childObjs',replacementY,'childCons',0,'matchedPairIds',1);
    [Retained,~] = PairBoundaryArchive_RC('update',Historical,Current, ...
        [Current,Replacement,VerticalInfeasible],LifecycleW,Problem, ...
        Options,Feedback,200);
    [RetainedData,~] = PairBoundaryArchive_RC( ...
        'trainingdata',Retained,LifecycleW,Problem,Options);
    assert(isscalar(Retained.id) && Retained.id == 1 && ...
        Retained.active && Retained.resumeEligible && Retained.age == 0 && ...
        Retained.lastFE == 123 && Retained.nextId >= 2 && ...
        containsDecision(Retained.xf,lifecycleXf) && ...
        any(RetainedData.id == 1));

    % A fresh elite/new-xi combination in the same occupied direction may
    % not bypass strict historical endpoint tightening during final prune.
    Protected = makeArchive([0.10 0.10],[0.50 0.50], ...
        [0.10 0.90],[0.20 0.80],1);
    Protected.lastFE = 123;
    BypassP1 = SOLUTION([0.90 0.90],[0.10 0.90],0);
    BypassXi = SOLUTION([0.92 0.92],[0.20 0.80],1);
    [Protected,~] = PairBoundaryArchive_RC('update',Protected,BypassP1, ...
        [BypassP1,BypassXi],LifecycleW,Problem,Options,struct(),250);
    assert(isscalar(Protected.id) && Protected.id == 1 && ...
        Protected.active && Protected.resumeEligible && ...
        Protected.lastFE == 123 && Protected.nextId == 2 && ...
        containsDecision(Protected.xf,[0.10 0.10]) && ...
        containsDecision(Protected.xi,[0.50 0.50]));

    % Any truly feasible row in the new P1 may replace historical xf when
    % it strictly shortens the pair, even when its current Fitness is >= 1.
    NewP1 = [Current,Replacement];
    replacementFitness = CalFitness_CBS(NewP1.objs,NewP1.cons);
    oldGap = Retained.gap;
    [Replaced,~] = PairBoundaryArchive_RC('update',Retained,NewP1, ...
        [NewP1,VerticalInfeasible],LifecycleW,Problem,Options,Feedback,300);
    assert(replacementFitness(2) >= 1 && isscalar(Replaced.id) && ...
        Replaced.id == 1 && Replaced.active && Replaced.age == 0 && ...
        Replaced.lastFE == 300 && Replaced.nextId >= 2 && ...
        containsDecision(Replaced.xf,replacementX) && Replaced.gap < oldGap);
    ...
    ReassociationW = twoObjectiveWeights(11);
    Reassociated = makeArchive(lifecycleXf,lifecycleXi, ...
        lifecycleYf,lifecycleYi,size(ReassociationW,1));
    Reassociated.lastFE = 123;
    [Reassociated,~] = PairBoundaryArchive_RC('update',Reassociated, ...
        ResumingP1,[ResumingP1,HorizontalInfeasible],ReassociationW,Problem, ...
        Options,struct(),600);
    ...
    assert(isequal(Selected,g) && Matched == 1 && ...
        ...
        Trace.constraintFE == 0 && Trace.spherePassCount == 1);
    ...
    % Both endpoints and the orthogonal sphere boundary are included; a
    % point just beyond that boundary is rejected.
    ballXf = [0.20 0.20];
    ballXi = [0.80 0.80];
    ballRaw = [ballXf;ballXi;0.50 0.50;0.80 0.20;0.81 0.19];
    ballObjective = CandidateProblem.CalObj(ballXf);
    BallArchive = makeArchive(ballXf,ballXi, ...
        ballObjective,ballObjective,1);
    BallInfo = struct('refs',ones(5,1),'pairIds',ones(5,1));
    BallScale = struct('minimum',ballObjective-1,'span',[2 2]);
    [BallSelected,~,~,BallTrace] = PairBoundaryArchive_RC( ...
        'selectcandidates',ballRaw,BallInfo,BallArchive,BallScale, ...
        CandidateProblem,pairOptions(1,2));
    assert(isequal(BallSelected,ballXf) && ...
        BallTrace.invalidCount == 0 && BallTrace.matchFailures == 0 && ...
        BallTrace.spherePassCount == 4 && ...
        BallTrace.sphereRejectCount == 1 && ...
        BallTrace.objectiveCandidateCount == 2 && BallTrace.keepIdx == 1);

    %% Closed pair-diameter ball, all passing raw, 2Q checks, Q donors
    pairCount = 5;
    W5 = twoObjectiveWeights(pairCount);
    Xf = [linspace(0.10,0.50,pairCount)',0.20*ones(pairCount,1)];
    Xi = Xf+0.25;
    Centers = 0.5*(Xf+Xi);
    Y = CandidateProblem.CalObj(Centers);
    ...
    [Selected,SelectedRefs,Matched,Trace] = ...
        ...
        ManyArchive,Scale,CandidateProblem,ManyOptions);
    assert(CandidateProblem.FE == feBefore);
    assert(Trace.objectiveCandidateCount == 4 && Trace.objectiveFE == 4);
    assert(Trace.spherePassCount == 2*pairCount && ...
        Trace.sphereRejectCount == 0 && Trace.keptCount == 2);
    assert(numel(unique(Matched)) == 2 && ...
        isequal(SelectedRefs,ManyArchive.ref(Matched)));
    assert(max(abs(Selected-Centers(Matched,:)),[],'all') <= 1e-12);
    assert(all(mod(Trace.keepIdx,2) == 1));
    LimitedOptions = pairOptions(2,3);
    [~,~,~,LimitedTrace] = PairBoundaryArchive_RC( ...
        'selectcandidates',Raw,ManyInfo,ManyArchive,Scale, ...
        CandidateProblem,LimitedOptions);
    ...
    [OneSelected,~,~,OneTrace] = PairBoundaryArchive_RC( ...
        'selectcandidates',Raw,ManyInfo,ManyArchive,Scale, ...
        CandidateProblem,OneObjectiveOptions);
