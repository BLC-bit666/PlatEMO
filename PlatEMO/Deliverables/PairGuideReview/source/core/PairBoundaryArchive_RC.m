...
%   resumeEligible records whether historical lifecycle continuity remains.
...
%UPDATEARCHIVE Refresh legal pairs from all truly evaluated information.
    ...
    [P1FeasX,P1FeasY,~,~] = evaluatedRows( ...
        Population1,zeros(0,Problem.D),Problem,Options);
    [FeedbackX,FeedbackY,FeedbackC,FeedbackIds] = ...
        validFeedbackRows(Feedback,Problem);
    if ~isempty(FeedbackX)
        feedbackFeasible = constraintViolation(FeedbackC) <= 0;
        retainedFeedback = ~feedbackFeasible | ...
            minimumNormalizedDistance(FeedbackX,P1FeasX,Problem) <= ...
            Options.pairDuplicateTolerance;
        FeedbackX = FeedbackX(retainedFeedback,:);
        FeedbackY = FeedbackY(retainedFeedback,:);
        FeedbackC = FeedbackC(retainedFeedback,:);
        FeedbackIds = FeedbackIds(retainedFeedback);
    end
    [~,~,InfX,InfY] = evaluatedRows( ...
    ...
    [Archive,feedbackTrace,~,forcedInactive,NewPairs] = ...
    ...
    [Archive,evaluatedTrace,~] = ...
        tightenRetainedPairs(Archive,P1FeasX,P1FeasY,InfX,InfY, ...
        EliteY,W,RefScale,Problem,currentFE,Options, ...
        continuable & ~forcedInactive);
    Archive.resumeEligible(forcedInactive) = false;
    Trace.guidedTightenedFeasible = feedbackTrace.tightenedFeasible;
    Trace.guidedTightenedInfeasible = feedbackTrace.tightenedInfeasible;
    Trace.ordinaryTightenedFeasible = evaluatedTrace.tightenedFeasible;
    Trace.ordinaryTightenedInfeasible = evaluatedTrace.tightenedInfeasible;
    Trace.tightenedFeasible = Trace.guidedTightenedFeasible+ ...
        Trace.ordinaryTightenedFeasible;
    Trace.tightenedInfeasible = Trace.guidedTightenedInfeasible+ ...
        Trace.ordinaryTightenedInfeasible;
    ...
    % A legal historical pair remains valuable training evidence even when
    % its feasible endpoint is no longer a current P1 Fitness<1 elite.
    Archive.active = Archive.resumeEligible & currentlyLegal & ...
        ~forcedInactive;
    Archive.age(Archive.active) = 0;
    Archive.age(~Archive.active) = Archive.age(~Archive.active)+1;

    % A legal historical direction is updated only through the strict-gap
    % tightening paths above. Fresh pairs may populate directions that do
    % not currently have an active pair, but may not bypass that rule by
    % competing with a legal historical pair during canonical pruning.
    occupiedRefs = unique(Archive.ref(Archive.active));
    for row = 1 : size(NewPairs.xf,1)
        if ismember(NewPairs.ref(row),occupiedRefs)
            continue;
        end
    ...
    for elite = 1 : size(EliteX,1)
        if ismember(EliteRef(elite),occupiedRefs)
            continue;
        end
    ...
    Trace.resumeEligible = nnz(Archive.resumeEligible);
...
function [SelectedDecs,SelectedRefs,MatchedIds,Trace] = ...
        selectCandidates(RawDec,SampleInfo,Archive,RefScale,Problem,Options)
%SELECTCANDIDATES Pair-diameter ball followed by objective-only legality.
    ...
    validRaw = all(isfinite(RawDec),2) & isfinite(refs) & ...
        refs == fix(refs) & refs >= 1 & ...
        isfinite(pairIds) & pairIds == fix(pairIds) & pairIds > 0 & ...
        all(RawDec >= lower-1e-12,2) & all(RawDec <= upper+1e-12,2);
    Trace.invalidCount = nnz(~validRaw);
    rawPairRows = zeros(rowCount,1);
    xfNorm = (Archive.xf-lower)./span;
    xiNorm = (Archive.xi-lower)./span;
    centers = 0.5*(xfNorm+xiNorm);
    radius2 = 0.25*sum((xiNorm-xfNorm).^2,2);
    matchFailures = 0;
    sphereRejectCount = 0;
    for raw = reshape(find(validRaw),1,[])
        ...
        distance2 = sum((Xn(raw,:)-centers(pair,:)).^2,2);
        if distance2 > radius2(pair)+1e-12
            sphereRejectCount = sphereRejectCount+1;
            continue;
        end
        rawPairRows(raw) = pair;
    end

    spherePassed = rawPairRows > 0;
    Trace.matchFailures = matchFailures;
    Trace.sphereRejectCount = sphereRejectCount;
    Trace.spherePassCount = nnz(spherePassed);
    sphereRows = find(spherePassed);
    if isempty(sphereRows)
    ...
    objectiveLimit = min([numel(sphereRows),2*quota, ...
        Options.objectiveBudget]);
    shortlist = sphereRows(1:objectiveLimit);
    ...
    Trace.jointPassCount = nnz(passed);
