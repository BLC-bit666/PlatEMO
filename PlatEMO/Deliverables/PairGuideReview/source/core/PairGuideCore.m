...
%--------------------------------------------------------------------------
    ...
    methods(Access = protected)
        ...
        %ALGORITHMCONFIGURATION Parse and lock PairGuide parameters.
            ...
            Config.trainingSigma = 1;
            Config.sampleSigma = 0.3;
    ...
    methods(Access = private)
        ...
        %CAPTUREFIRSTPAIRGUIDESIGMAPOOLS Replay inference sigma only.
            ...
            if S.sampleSigmas(1) ~= PairOptions.sampleSigma
                error('CBSPairGuide:UnpairedPrimarySigma', ...
                    ['The first paired sample sigma must equal the ', ...
                     'production sample sigma.']);
            end
            mainRngState = rng;
            ...
            for arm = 1 : numel(S.sampleSigmas)
                ...
                [SelectedDecs,SelectedRefs,MatchedIds,PoolTrace] = ...
                    ...
                    Info,PairArchive,RefScale,Problem,PairOptions);
            ...
            if ~primaryMatches
                error('CBSPairGuide:PrimarySigmaReplayMismatch', ...
                    'Primary-sigma pool replay changed the production sample.');
            end
            S.sigmaArms = Arms;
        ...
        %CAPTUREFIRSTPAIRGUIDESIGMACHILDREN Replay only guided decisions.
            ...
            if ~primaryMatches
                error('CBSPairGuide:PrimaryChildReplayMismatch', ...
                    'Primary-sigma child replay changed the production guide.');
            end
            S.sigmaArms = Arms;
            Algorithm.FirstPairGuideUseState = S;
            clear cleanup;
...
%RECORDFIRSTPAIRGUIDEUSE Capture parents and true children before Pop2 use.
        ...
        %AUDITNOTTERMINATED Save behavior-neutral population checkpoints.
            ...
            if double(Problem.FE) >= double(Problem.maxFE)
            ...
            while Algorithm.auditNotTerminated(Population1,Problem,W)
                ...
                if pairPoolReady
                    ...
                    [GuideDecs,GuideRefs,GuideSlots,PoolTrace] = ...
                        ...
                        RawGuideDecs,SampleInfo,PairArchive,pairRefScale, ...
                        Problem,SelectionOptions);
                    ...
                    activePairRows = logical(BMem.active);
                    Algorithm.recordObjectiveSnapshotPool( ...
                        RawGuideDecs,Problem.FE, ...
                        BMem.x_b(activePairRows,:), ...
                        BMem.x_i(activePairRows,:));
...
%PAIRGUIDEDECISIONS Convert accepted donors into deterministic midpoints.
    ...
    Map = struct('validCount',0,'droppedCount',size(GuideDecs,1), ...
        'noValidScale',false,'centerStep',zeros(0,1), ...
...
%EMPTYOBJECTIVESNAPSHOTSTATE Initialize optional target-FE point capture.
    ...
    Snapshot = struct('targetFE',NaN,'actualFE',NaN,'poolFE',NaN, ...
        ...
        'archiveFeasibleObjs',zeros(0,Problem.M), ...
        'archiveFeasibleCons',zeros(0,0), ...
        'archiveInfeasibleObjs',zeros(0,Problem.M), ...
        'archiveInfeasibleCons',zeros(0,0), ...
        'archiveFeasibleDecs',zeros(0,Problem.D), ...
        'archiveInfeasibleDecs',zeros(0,Problem.D), ...
...
%VALIDATEFIRSTPAIRGUIDEUSEOPTIONS Validate the isolated epoch probe.
    ...
    if any(~isfinite(sampleSigmas) | sampleSigmas < 0) || ...
        ...
        error('CBSPairGuide:BadUseCaptureSigmas', ...
            'sampleSigmas must be unique, finite, and nonnegative.');
    end
...
%MAKEOBJECTIVESNAPSHOT Evaluate display-only points without changing search.
    ...
    ArchiveF = auditOptionalEvaluation(Problem,Event.trainXf);
    ArchiveI = auditOptionalEvaluation(Problem,Event.trainXi);
    ...
    Snapshot = struct('targetFE',double(targetFE), ...
        ...
        'archiveFeasibleObjs',solutionObjectives(ArchiveF,Problem.M), ...
        'archiveFeasibleCons',solutionConstraints(ArchiveF), ...
        'archiveInfeasibleObjs',solutionObjectives(ArchiveI,Problem.M), ...
        'archiveInfeasibleCons',solutionConstraints(ArchiveI), ...
        'archiveFeasibleDecs',double(Event.trainXf), ...
        'archiveInfeasibleDecs',double(Event.trainXi), ...
