function varargout = PairBoundaryWGAN_UniformGapProbe(action,varargin)
...
%GENERATORGRADIENTS Ladv + 10 Lend + Ldelta, relative to each true pair gap.
    ...
    relativeGapSquared = max(sum((targetI-targetF).^2,1),single(1e-6*size(targetF,1)));
    gapSquared = ones(1,size(targetF,2),'single')./mean(1./relativeGapSquared,'all');
