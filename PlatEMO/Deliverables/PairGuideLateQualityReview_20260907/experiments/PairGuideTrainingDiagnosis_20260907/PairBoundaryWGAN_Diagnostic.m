function varargout = PairBoundaryWGAN_Diagnostic(action,varargin)
...
%--------------------------------------------------------------------------
    ...
    % Isolated diagnostic copy: production defaults are unchanged.
    switch lower(strtrim(string(action)))
        ...
        case "lossprobe"
            varargout{1} = lossProbe(varargin{:});
        otherwise
...
%UPDATEGENERATOR Reuse the same paired output for adversarial and gap losses.
    ...
    gradients = dlfeval(@generatorGradients,Model.netG,Model.netC, ...
        cF,cI,targetF,targetI,zPair,Options);
    if ~all(cellfun(@(x)all(isfinite(extractdata(x)),'all'),gradients.Value))
...
function gradients = generatorGradients(netG,netC,cF,cI,targetF,targetI,zPair,Options)
%GENERATORGRADIENTS Ladv + 10 Lend + Ldelta, relative to each true pair gap.
    ...
    if isfield(Options,'uniformGap') && Options.uniformGap
        gapSquared = ones(size(gapSquared),'like',gapSquared)/mean(1./gapSquared);
    end
    ...
    direction = mean(sum(((generatedI-generatedF)-(targetI-targetF)).^2,1) ...
        ./gapSquared,'all');
    advWeight=1; if isfield(Options,'noAdv') && Options.noAdv, advWeight=0; end
    loss = advWeight*adversarial+10*endpoint+direction;
...
function S=lossProbe(Model,Data)
% Norms refer to the same full fixed batch, zero latent noise, and current networks.
C=dlarray(single([Data.cF;Data.cI]'),'CB');
Z=dlarray(zeros(Model.zDim,2*Data.count,'single'),'CB');
F=dlarray(single(Data.xF'),'CB'); I=dlarray(single(Data.xI'),'CB');
values=zeros(1,3); vectors=cell(1,3);
for mode=1:3
 [v,g]=dlfeval(@probeGradient,Model.netG,Model.netC,C,Z,F,I,mode);
 values(mode)=double(extractdata(v));
 vals=cellfun(@(x)double(extractdata(x(:))),g.Value,'UniformOutput',false);
 vectors{mode}=vertcat(vals{:});
end
geom=10*vectors{2}+vectors{3};
S=struct('adversarialLoss',values(1),'endpointLoss',values(2),'directionLoss',values(3), ...
 'adversarialGradientNorm',norm(vectors{1}),'endpointGradientNorm',norm(10*vectors{2}), ...
 'directionGradientNorm',norm(vectors{3}),'geometryGradientNorm',norm(geom), ...
 'advGeometryCosine',dot(vectors{1},geom)/max(eps,norm(vectors{1})*norm(geom)), ...
 'generatorParameters',numel(geom));
end
function [loss,g]=probeGradient(G,C,cond,z,F,I,mode)
y=forward(G,[z;cond]); n=size(F,2); gf=(y(:,1:n)+1)/2; gi=(y(:,n+1:end)+1)/2;
gap=max(sum((I-F).^2,1),single(1e-6*size(F,1)));
if mode==1, loss=-mean(forward(C,[y;cond]),'all');
elseif mode==2, loss=mean((sum((gf-F).^2,1)+sum((gi-I).^2,1))./(2*gap),'all');
else, loss=mean(sum(((gi-gf)-(I-F)).^2,1)./gap,'all'); end
g=dlgradient(loss,G.Learnables);
end
