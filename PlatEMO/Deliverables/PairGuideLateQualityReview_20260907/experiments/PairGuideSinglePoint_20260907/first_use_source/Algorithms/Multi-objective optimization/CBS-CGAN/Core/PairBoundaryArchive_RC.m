...
%SELECTCANDIDATES Keep original points, balanced by requested direction.
    if isfield(O,'pairOnly') && O.pairOnly
        [Dec,Refs,Ids,T] = selectPairedCandidates(X,Info,Archive,Scale,Problem,O);
        return;
    end
    O = fillOptions(O); n = size(X,1); T = emptyPoolTrace();
    ...
    if isfield(O,'W'); nr = size(O.W,1); else; nr = max([0;Info.refs]); end
    valid = valid & isfinite(Info.refs) & Info.refs == fix(Info.refs) & Info.refs >= 1 & Info.refs <= nr;
    ...
    refs = unique(Info.refs(valid)); refs = refs(randperm(numel(refs)));
    chosen = zeros(0,1); remaining = valid;
    while numel(chosen) < O.guideQuota && any(remaining)
        before = numel(chosen);
        for ref = reshape(refs,1,[])
            for k = reshape(find(remaining & Info.refs == ref),1,[])
                remaining(k) = false;
                if ~isempty(base) && any(vecnorm(base-Z(k,:),2,2) <= O.pairDuplicateTolerance)
                    continue;
                end
    ...
    Dec = double(X(chosen,:)); Refs = Info.refs(chosen); Ids = zeros(numel(chosen),1);
    T.keepIdx = chosen; T.keptCount = numel(chosen); T.keptConditions = numel(unique(Refs));
    T.matchedPairIds = Ids; T.rawPairIds = zeros(n,1); T.rawRefs = Info.refs;
    T.rawSides = Info.sides; T.selectedSides = Info.sides(chosen);
    % Pair-relative diagnostics are unavailable for unpaired proposals.
    ...
    T.spherePassCount = nnz(valid); T.jointPassCount = nnz(valid);
