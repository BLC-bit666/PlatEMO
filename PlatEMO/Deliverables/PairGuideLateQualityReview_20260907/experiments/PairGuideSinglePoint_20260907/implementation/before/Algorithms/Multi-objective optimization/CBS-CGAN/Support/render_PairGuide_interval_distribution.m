...
%   Additional CalObj calls are cached and billed separately; no CalCon call.
    ...
    if isfile(cacheFile)
        ...
        if isfield(old.Report,'rendererVersion') && old.Report.rendererVersion==3 && ...
    ...
    E=R.Audit.evidence;
    nativeMode=E.schema=="PairGuide-native-v2";
        ...
        if indices(k)>1
            ...
            if ~isempty(q)
                ...
                if isempty(queryCache{q})
                    ...
                    Y=nan(size(X,1),Problem.M);
                    [uniqueX,~,map]=unique(X(valid,:),'rows');
                    if ~isempty(uniqueX); uniqueY=Problem.CalObj(uniqueX); Y(valid,:)=uniqueY(map,:); end
                    queryCache{q}=struct('native',Y(1:size(raw,1),:), ...
                        'candidates',Y(size(raw,1)+1:end,:));
                end
    ...
    Report=struct('rendererVersion',3,'sourceHash',R.Record.sourceHash,'problem',R.Record.problem, ...
...
function renderFigure(Problem,problem,S,label,file,paired,focused)
    ...
    title(layout,{sprintf('%s | cgan run 1 | %s | target FE %g | observed FE %g', ...
        ...
        sprintf('production FE %g | consumption FE %g | model %g | stale=%d | raw objectives: offline only', ...
        S.productionFE,S.consumptionFE,S.modelVersion,S.stale), ...
        sprintf('Last training: FE %g | %g pairs | %g epochs / %g G updates | endpoint RMSE %.4g', ...
        S.trainingFE,S.trainingPairs,S.trainingEpochs,S.trainingUpdates,S.trainingRMSE),note}, ...
