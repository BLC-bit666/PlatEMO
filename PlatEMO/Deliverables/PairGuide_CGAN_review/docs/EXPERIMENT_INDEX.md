...
| 目录 | 核心问题 | 协议 | 关键资料 |
|---|---|---|---|
| `first_use_epoch_v4_20260901` | 首次训练 Epoch 对首次生成/使用形状和质量的影响 | 6 问题；Epoch 400/600/800/1000/1200；2 runs；首次使用后停止 | 完整 `analysis/*.csv`；附件内选取 400/1000、run01 代表图；`figure_index.csv` 记录完整图集 |
| `training_schedule_v1_20260901` | Critic:Generator 与后续 Epoch | 6 问题；30 个配对块；先比较 nCritic 1/2/4，再比较重训 5/10/20 | 两阶段统计汇总、全部 3×10 放大细节图；精简附件不含约 20 MB 逐事件诊断 CSV |
...
- v4 在更新后的范围 400–1200 上按既定综合规则推荐 1000；精简附件仅裁取 400/1000 的 run01 图片，但统计表仍覆盖全部设置。
