...
% The production path remains frozen. The PairGuide comparator reuses this
% engine but learns atomic feasible/infeasible endpoint pairs, queries the
% infeasible side, and converts generated points into local DE directions.
            ...
            while Algorithm.auditNotTerminated(Population1,Problem,W)
                ...
                if pairPoolReady
                    ...
                    [GuideDecs,GuideRefs,GuideSlots,PoolTrace] = ...
                        ...
                        Fitness1,W,pairRefScale,Problem,PairOptions);
                    ...
                    Algorithm.captureFirstPairGuideSigmaPools(Problem,GAN, ...
                        ...
                        W,pairRefScale,PairOptions,sampleRngState);
                    if ~isempty(GuideDecs)
...
%   Before the cutoff, retained legacy entries select their configured
%   CGAN path, while DE20 replaces the same quota. PairGuide has no fixed
%   cutoff and uses only its gated pair archive/model.
...
%PAIRGUIDEDECISIONS Map every G to local T, maximin-select, then DE-evaluate.

    tau = 1e-12;
    Map = struct('validCount',0,'droppedCount',size(GuideDecs,1), ...
        'noValidScale',false,'Fg',zeros(0,1),'h',zeros(0,1), ...
    ...
    hAll = robustLocalScale(EliteNorm,tau,1);
    ...
    Sources = zeros(rowCount,1);
    Refs = zeros(rowCount,1);
    PairIds = zeros(rowCount,1);
    Fg = zeros(rowCount,1);
    H = zeros(rowCount,1);
    Distance = zeros(rowCount,1);
    mapped = 0;
    for source = 1 : rowCount
        ...
        if isempty(pair) || any(~isfinite(G)) || ...
                any(G < lower-1e-12) || any(G > upper+1e-12)
            continue;
        end
        ...
        % This is a local attribution/use gate, not archive admission.
        if norm(FeasNorm(parent,:)-xfNorm) > ...
                Config.pairSupportMultiplier*h
            continue;
        end
        ...
        scale = min(0.5,h/(d+eps));
        T = min(max(A+scale*(G-A),lower),upper);
        mapped = mapped+1;
        Targets(mapped,:) = T;
        Parents(mapped) = parent;
        Sources(mapped) = source;
    ...
    Targets = Targets(1:mapped,:);
    Parents = Parents(1:mapped);
    Sources = Sources(1:mapped);
    Refs = Refs(1:mapped);
    PairIds = PairIds(1:mapped);
    Fg = Fg(1:mapped);
    H = H(1:mapped);
    Distance = Distance(1:mapped);
    Map.validCount = mapped;
    Map.droppedCount = rowCount-mapped;
    if mapped == 0
        return;
    end
    selectionTolerance = max(tau,double(Config.pairDuplicateTolerance));
    selected = vacancyWeightedMaximin(Problem,Targets,Refs, ...
        FeasDecs(elite,:),feasRefs(elite),W,guidedCount, ...
        selectionTolerance);
    if isempty(selected)
        return;
    end
    parentRows = Parents(selected);
    selectedRefs = Refs(selected);
    matchedPairIds = PairIds(selected);
    A = FeasDecs(parentRows,:);
    T = Targets(selected,:);
    ChildDecs = OperatorDE(Problem,A,T,A,{1,1,1,20});
    Map.Fg = Fg(selected);
    Map.h = H(selected);
    Map.d = Distance(selected);
    Map.auditTargets = GuideDecs(Sources(selected),:);
    Map.auditCenters = T;
...
%CLOSESTREFERENCEROWS Reference plus nearest requested directions.

    distance = sqrt(sum((double(W)-double(W(ref,:))).^2,2));
    [~,order] = sortrows([distance,(1:size(W,1))'],[1 2]);
    rows = order(1:min(size(W,1),1+max(0,round(double(count)))));
end
...
%PAIRGANOPTIONS Isolate PairGuide archive and WGAN settings from mainline.
    ...
    names = ["zDim","pairGanEpoch","pairInitialEpoch", ...
        ...
        "pairRelationWeight", ...
        "pairLatentWeight","pairLatentThreshold", ...
        "pairDuplicateTolerance","pairImprovementTolerance", ...
        "pairSupportMultiplier"];
    Options = struct();
    for name = names
        field = char(name);
...
%EMPTYGUIDESTATS Initialize aggregate experiment counters.
    ...
    S.pairGuideSchemaVersion = "CBS-CGAN-pair-guide-v4";
...
%EMPTYPAIRTRAININGEVENT One behavior-neutral incremental-training record.
    ...
    Event = struct('fe',NaN,'kind',"",'nCritic',NaN,'epochs',NaN, ...
        ...
        'preAllRelationRMSE',NaN,'preChangedRelationRMSE',NaN, ...
        'preCriticGap',NaN,'preGeneratedNearestMedian',NaN, ...
        'preGeneratedSpread',NaN,'postAllRelationRMSE',NaN, ...
        'postChangedRelationRMSE',NaN,'postCriticGap',NaN, ...
        'postGeneratedNearestMedian',NaN,'postGeneratedSpread',NaN);
end
...
%COPYPAIRMODELDIAGNOSTICS Copy fixed-probe diagnostics into one log row.
    ...
    names = {'allRelationRMSE','changedRelationRMSE','criticGap', ...
        'generatedNearestMedian','generatedSpread'};
    for i = 1 : numel(names)
