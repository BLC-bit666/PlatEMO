classdef CBS_RegionWGAN_GP_PairGuide_Quota40 < CBS_RegionWGAN_GP_PairGuide
% <2026> <multi> <real> <constrained>
% PairGuide experiment using a 40% constrained-population offspring quota

%------------------------------- Copyright --------------------------------
% Copyright (c) 2026 BIMK Group. You are free to use PlatEMO for research.
%--------------------------------------------------------------------------

    methods(Access = protected)
        function Config = algorithmConfiguration(Algorithm)
            Config = algorithmConfiguration@CBS_RegionWGAN_GP_PairGuide( ...
                Algorithm);
            Config.experimentArm = 40;
            Config.guideOffspringShare = 0.40;
        end
    end
end
