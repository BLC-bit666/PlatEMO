function run_stage_suffixes(nWorker)
...
folder=fileparts(mfilename('fullpath')); addpath(fullfile(root,'Data','PairGuideGPTValidation_20260908')); addpath(folder);
...
for number=5:8
    for stage=1:3
        for mode=["raw","adversarial20","distance20","adversarial2000","distance2000","de","pair_only"]
            Tasks(end+1)=struct('number',number,'seed',1,'stage',stage,'mode',mode);
        end
    end
end
if nWorker==0
    suffix_case(root,folder,out,Tasks(1));
    suffix_case(root,folder,out,Tasks(2)); fprintf('SUFFIX_SMOKE_COMPLETE\n'); return;
end
...
fprintf('STAGE_SUFFIX_CASES_COMPLETE\n');
end

function suffix_case(root,folder,out,Task)
maxNumCompThreads(1); addCBSPaths(root); addpath(fullfile(root,'Data','PairGuideGPTValidation_20260908')); addpath(folder);
name=sprintf('LIRCMOP%d_BC',Task.number); fe=[20000 50000 80000]; fe=fe(Task.stage);
stem=sprintf('%s_seed%02d_FE%06d_%s',name,Task.seed,fe,Task.mode);
target=fullfile(out,[stem,'.mat']); if isfile(target); return; end
labels={'early','middle','late'};stage=labels{Task.stage};
Cached=load(fullfile(folder,'suffix_fixtures',sprintf('%s_seed%02d_%s.mat',name,Task.seed,stage)));
Prefix=Cached.Prefix;S=Prefix.state;W=Cached.W;
...
Fit1=Cached.Fitness1; Fit2=Cached.Fitness2;
...
Pending=Prefix.pending; OriginalQ=zeros(0,P.D); if isfield(Pending,'decs');OriginalQ=Pending.decs;end
ParentIndices=[];
if startsWith(Task.mode,"adversarial") || startsWith(Task.mode,"distance")
    if Prefix.queryAvailable
        if startsWith(Task.mode,"adversarial");loss='adversarial';else;loss='distance';end
        updates=str2double(erase(Task.mode,string(loss)));
        sourceModel=fullfile(folder,'runs',sprintf('%s_seed01_%s_%s_stream1.mat',name,stage,loss));
        Trained=load(sourceModel,'Snapshots','Table','Complete'); assert(Trained.Complete);
        ix=find(Trained.Table.addedUpdates==updates,1); assert(~isempty(ix));
        saved=rng;
        % Historical evidence predates the optional query.conditionScale field.
        % Sampling uses the saved model and exact query conditions; this scale
        % is reported as metadata only by the native sampler.
        Q=Prefix.query;O=struct('sampleSigma',0,'referenceScale',S.referenceScale);
        [OriginalRaw,~]=PairBoundaryWGAN_RC('sample',Trained.Snapshots{1}.Model,Q.conditions,P,O);
        assert(isequal(OriginalRaw(Q.pool.keepIdx,:),OriginalQ));
        [Raw,~]=PairBoundaryWGAN_RC('sample',Trained.Snapshots{ix}.Model,Q.conditions,P,O);
        rng(saved);
        Pending.decs=Raw(Q.pool.keepIdx,:);
        assert(all(isfinite(Pending.decs),'all') && all(Pending.ids==0));
        % Keep original requested directions/order; native consumption still checks
        % duplicates/bounds and fills missing slots with the same DE mechanism.
    end
elseif Task.mode=="de"
    Pending=struct();
...
R=struct('problem',string(name),'seed',Task.seed,'prefixFE',fe,'mode',Task.mode,'backboneSeed',backboneSeed, ...
    'queryAvailable',Prefix.queryAvailable,'trainingStream',1,'initialIGD',Trajectory(1,2),'finalIGD',Trajectory(end,2),'finalHV',Trajectory(end,3), ...
