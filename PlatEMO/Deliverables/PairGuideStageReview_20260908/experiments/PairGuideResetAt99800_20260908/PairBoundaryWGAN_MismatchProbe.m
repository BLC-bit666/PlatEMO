function varargout = PairBoundaryWGAN_MismatchProbe(action,varargin)
...
%UPDATECRITIC Conditional Wasserstein difference and decision gradient penalty.
    ...
    % Diagnostic intervention: half generated negatives, half real endpoints
    % with a different direction but the same side. Preserve all RNG draws.
    perm = 1:n;
    for side = unique(Cond(end,:))
        ix = find(Cond(end,:)==side);
        [~,order] = sort(Cond(1,ix)); ix = ix(order);
        perm(ix) = circshift(ix,floor(numel(ix)/2));
    end
    negative = fake;
    useMismatch = mod((1:n)+Model.iterC,2)==0;
    negative(:,useMismatch) = Real(:,perm(useMismatch));
