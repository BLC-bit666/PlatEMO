function run_fresh_conditions(nWorker)
% Same frozen late data and batches, fresh weights and Adam vs cached warm arm.
...
out=fullfile(folder,'fresh_conditions'); if ~isfolder(out); mkdir(out); end
...
assert(all(strlength(errors)==0)); fprintf('FRESH_CONDITIONS_COMPLETE\n');
end
...
stem=sprintf('%s_seed%02d_fresh',name,task.seed);
...
Data=F.Data; Model=fresh_model(F.Model,300000+task.seed); W=F.W;
...
Warm=load(fullfile(folder,'fixed_conditions',sprintf('%s_seed%02d_moving0.mat',name,task.seed)),'Schedule');
Schedule=Warm.Schedule;assert(isequal(size(Schedule),[2000,batch]));
assert(all(Schedule>=1 & Schedule<=numel(keep),'all'));
...
for event=1:100
    ...
    if ismember(event,[1 10 50 100])
        ...
        fprintf('FRESH_CASE %s updates=%d RMSE=%.4g angle=%.2f\n',stem,20*event, ...
