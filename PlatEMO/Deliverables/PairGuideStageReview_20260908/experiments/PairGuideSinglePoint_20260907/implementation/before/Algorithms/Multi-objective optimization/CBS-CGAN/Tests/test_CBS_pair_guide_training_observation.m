...
%TEST_CBS_PAIR_GUIDE_TRAINING_OBSERVATION Delayed direct evaluation and pure logging.
    ...
    assert(E.networkEndpointRows == 1000*numel(E.queries));
    assert(E.schema == "PairGuide-native-v2");
    ...
    for k = 1:numel(E.generations)
        ...
        assert(all(arrayfun(@(id)nnz(U.matchedPairIds == id),unique(U.matchedPairIds)) <= 2));
