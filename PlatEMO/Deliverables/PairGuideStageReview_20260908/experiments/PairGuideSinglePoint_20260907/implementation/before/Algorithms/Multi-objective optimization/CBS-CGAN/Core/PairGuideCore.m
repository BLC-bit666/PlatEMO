...
% atomic real endpoint pairs and directly uses native CGAN candidates.
...
%--------------------------------------------------------------------------
    ...
    methods(Static)
        ...
        %MAINLINEDEFAULTS Return public parameters and fixed mainline values.
            ...
            Defaults.pairGanEpoch = 200;
            Defaults.pairInitialEpoch = 200;
    ...
    methods(Access = private)
        ...
        %RUNMAINLINE CCMO selection, complete archive Union, delayed guidance.
            ...
            GAN = [];
            generation = 0;
            Evidence = struct('schema',"PairGuide-native-v2",'mode',Algorithm.ComparisonMode, ...
            ...
            timer = tic;
            [Archive,~,~] = PairBoundaryArchive_RC('update',[],Population1, ...
                ...
                if Options.guideQuota > 0 && Algorithm.ComparisonMode ~= "fallback_only"
                    ...
                    if ready && Config.rawGuideCount > 0
                        ...
                        Pending = struct('decs',Decs,'refs',Refs,'ids',Ids, ...
                            ...
                            'productionFE',double(Problem.FE),'productionGeneration',generation, ...
                        ...
                        Evidence.queries{end+1} = struct('productionFE',double(Problem.FE), ...
                            ...
                            'rawDecs',Raw,'sample',Sample,'pool',Pool,'pending',Pending);
                    end
                end
    ...
    if isfield(Pending,'decs')
        ...
        for k = 1:size(Pending.decs,1)
            ...
            if any(~isfinite(q)) || any(q < Problem.lower-1e-12) || ...
                    ...
                    nnz(Pending.ids(selected) == Pending.ids(k)) >= 2
                continue;
            end
