...
%TEST_CBS_PAIR_GUIDE Final shared-conversation contract, including real autodiff.
    ...
    assert(M.ready && S.updates == 2 && S.criticUpdates == 2 && ...
        S.pairVisits == 16 && isfinite(S.preDiagnostics.allEndpointRMSE));
    [Raw,Sample] = PairBoundaryWGAN_RC('sample',M,C,P,O);
    expected = P.lower+((1-Sample.alpha).*Sample.generatedF+ ...
        Sample.alpha.*Sample.generatedI).*(P.upper-P.lower);
    assert(isequal(Raw,expected) && Sample.endpointForwardRows == 1000 && ...
        size(Sample.z,1) == 500);
