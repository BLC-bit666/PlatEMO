...
| --- | --- | --- | --- | --- | --- |
| 200 | 1.9213 | 2.08 | 49.1% | 18.97° | 5.5% |
| 500 | 2.0860 | 3.83 | 56.6% | 14.58° | 13.9% |
| 1000 | 1.8420 | 3.92 | 55.4% | 15.63° | 10.8% |
| 2000 | 1.9561 | 3.17 | 53.3% | 15.77° | 14.1% |
| 4000 | 1.8531 | 3.75 | 55.6% | 14.31° | 13.2% |
...
数据：[全部首次使用结果](first_use_all.csv)、[统一预算汇总](selected_budget_summary.csv)、[联合指标](first_use_joint_scores.csv)、[前缀与消费核查](first_use_verification.json)。
...
| 问题 | 单点 CGAN | 普通 DE 替代 | 真实配对采样 | 旧配对 CGAN |
| --- | --- | --- | --- | --- |
| LIRCMOP5_BC | 0.015074 ± 0.001879 | 0.020489 ± 0.012121 | 0.014086 ± 0.001226 | 0.021447 ± 0.014110 |
| LIRCMOP6_BC | 0.013741 ± 0.000815 | 0.011902 ± 0.001701 | 0.014266 ± 0.004778 | 0.015694 ± 0.004180 |
| LIRCMOP7_BC | 0.008525 ± 0.000362 | 0.008988 ± 0.000407 | 0.006913 ± 0.000416 | 0.008779 ± 0.000651 |
| LIRCMOP8_BC | 0.007982 ± 0.000229 | 0.007956 ± 0.000473 | 0.006704 ± 0.000136 | 0.007701 ± 0.000270 |

最终 HV（均值±样本标准差，越高越好）：

| 问题 | 单点 CGAN | 普通 DE 替代 | 真实配对采样 | 旧配对 CGAN |
| --- | --- | --- | --- | --- |
| LIRCMOP5_BC | 0.286942 ± 0.000702 | 0.284512 ± 0.005246 | 0.287206 ± 0.001024 | 0.283823 ± 0.006524 |
| LIRCMOP6_BC | 0.193091 ± 0.000380 | 0.193699 ± 0.000955 | 0.193227 ± 0.001352 | 0.192241 ± 0.001368 |
| LIRCMOP7_BC | 0.293678 ± 0.000088 | 0.293227 ± 0.000301 | 0.295024 ± 0.000114 | 0.293401 ± 0.000562 |
| LIRCMOP8_BC | 0.294025 ± 0.000108 | 0.293834 ± 0.000330 | 0.294915 ± 0.000099 | 0.294077 ± 0.000206 |
...
数据：[逐 run 结果与成本](full_run/interval_summary.csv)、[同种子比较](full_run/analysis/paired_comparisons.csv)、[指定 FE 检查点](full_run/analysis/checkpoints_per_run.csv)。

## 4. 当前生成质量与偏移

100000 FE 的当代生成/消费事件，分别按生成时与模型训练时的目标尺度计算角度：

| 问题 | 有当代生成/消费证据的 runs | 标签正确率 | 生成尺度方向误差 | 训练尺度方向误差 | 可行且不被当前 P1 支配 |
| --- | --- | --- | --- | --- | --- |
| LIRCMOP5_BC | 3/3 | 48.3% | 22.39° | 19.97° | 0.0% |
| LIRCMOP6_BC | 1/3 | 47.8% | 26.27° | 24.77° | 0.0% |
| LIRCMOP7_BC | 3/3 | 54.7% | 9.58° | 9.58° | 0.3% |
| LIRCMOP8_BC | 3/3 | 51.7% | 5.72° | 5.72° | 0.7% |

该表对 raw 作独立真实评价；没有当代生成/消费对应事件的 run 明确缺测。近邻目标距离、已见/未见条件、各侧标签准确率及全部阶段结果见 [条件质量明细](conditional_quality.csv)。目标尺度变化的影响应看同一行两个角度的差异，不能把所有误差都归因于尺度漂移。
...
run 1 的末期完整图：LIRCMOP5：未见条件大幅偏移（历史原包文件：full_run/analysis/figures/LIRCMOP5_BC/run_01/LIRCMOP5_BC_FE100000_actualFE100000.png）、LIRCMOP6：当前存档为空（历史原包文件：full_run/analysis/figures/LIRCMOP6_BC/run_01/LIRCMOP6_BC_FE100000_actualFE100000.png）、LIRCMOP7：生成曲线仍在边界内侧（历史原包文件：full_run/analysis/figures/LIRCMOP7_BC/run_01/LIRCMOP7_BC_FE100000_actualFE100000.png）、LIRCMOP8：角度接近仍不保证边界位置（历史原包文件：full_run/analysis/figures/LIRCMOP8_BC/run_01/LIRCMOP8_BC_FE100000_actualFE100000.png）。

!LIRCMOP8：左为模型对应的真实训练端点，右为原生生成点；颜色是请求标签，金色为真实消费点（历史原包文件：full_run/analysis/figures/LIRCMOP8_BC/run_01/LIRCMOP8_BC_FE100000_actualFE100000.png）
...
| 问题 | 训练数据 | 区间方向误差：1000→4000 | 区间标签正确率 | 区间联合命中率 | 区间可行改进候选比例 |
| --- | --- | --- | --- | --- | --- |
| LIRCMOP7_BC | 保留全部方向 | 12.38° → 6.86° | 50.6% → 50.6% | 0.0% → 11.8% | 0.0% → 0.0% |
| LIRCMOP7_BC | 留出 17 个连续方向 | 20.17° → 3.83° | 50.6% → 50.6% | 0.0% → 50.6% | 0.0% → 0.0% |
| LIRCMOP8_BC | 保留全部方向 | 5.83° → 6.94° | 49.4% → 49.4% | 20.0% → 5.9% | 0.0% → 0.0% |
| LIRCMOP8_BC | 留出 17 个连续方向 | 5.42° → 4.01° | 49.4% → 49.4% | 11.8% → 37.6% | 0.0% → 0.0% |
...
数据：[1000 步留出对照](holdout/selected1000_summary.csv)、[续训至 4000 步](holdout/continued4000_summary.csv)。

生图：LIRCMOP7 留出与续训（历史原包文件：holdout/LIRCMOP7_BC_direction_holdout.png）、LIRCMOP8 留出与续训（历史原包文件：holdout/LIRCMOP8_BC_direction_holdout.png）。这些图只使用已缓存的真实目标，没有额外 CalObj/CalCon 调用。
...
证据：最终回归日志（历史原包文件：<implementation/final_regression.log>）、静态检查（历史原包文件：<implementation/final_static.log>）、[完整运行核查](full_verification.csv)、[条件审计独立账单](conditional_quality_offline_cost.csv)、[最终配置](selected_configuration.json)、[冻结源码清单](full_source_manifest.json)、[首训源码来源说明](first_source_provenance.csv)。
