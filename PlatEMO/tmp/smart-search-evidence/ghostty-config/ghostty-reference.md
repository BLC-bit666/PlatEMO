...
* [Financial Support](/docs/sponsor)
...
> This setting cannot be reloaded at runtime. To change the language you must fully restart Ghostty.
...
```
font-family = "" font-family = "My Favorite Font" 
```
...
> This only applies to text copying operations, not URL copying.
...
* `raw:` - Send raw text as-is. This uses Zig string literal syntax so you can specify control characters and other standard escapes.
...
Key tables are defined using the syntax `/. The  value is everything documented above for keybinds. The 

value is the name of the key table. Table names can contain anything except /, =, +, and >. The characters + and > are reserved for keybind syntax (modifier combinations and key sequences). For example foo/ctrl+a=new_window defines a binding within a table named foo.

Tables are activated and deactivated using the binding actions activate_key_table: and deactivate_key_table. Other table related binding actions also exist; see the documentation for a full list. These are the primary way to interact with key tables.
...
* / (with no binding) defines and clears a table, resetting all of its keybinds and settings.
...
* A table can be activated in one-shot mode using activate_key_table_once:. A one-shot table is automatically deactivated when any non-catch-all binding is invoked.
...
```
command-palette-entry 0command-palette-entry on oncommand-palette-entry"Focus the split to the right, if it exists."command-palette-entry "Ghostty""Add a little Ghostty to your terminal.""text:\xf0\x9f\x91\xbb"
```
