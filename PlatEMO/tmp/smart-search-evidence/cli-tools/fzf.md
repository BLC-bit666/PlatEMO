![fzf - a command-line fuzzy finder](https://raw.githubusercontent.com/junegunn/i/master/fzf-color.png)
[![Build Status](https://github.com/junegunn/fzf/actions/workflows/linux.yml/badge.svg?branch=master)](https://github.com/junegunn/fzf/actions)
[![Version](https://img.shields.io/github/v/tag/junegunn/fzf)](http://github.com/junegunn/fzf/releases)
[![License](https://img.shields.io/github/license/junegunn/fzf)](https://github.com/junegunn/fzf?tab=MIT-1-ov-file#readme)
[![Contributors](https://img.shields.io/github/contributors/junegunn/fzf)](https://github.com/junegunn/fzf/graphs/contributors)
[![Sponsors](https://img.shields.io/github/sponsors/junegunn)](https://github.com/sponsors/junegunn)
[![Stars](https://img.shields.io/github/stars/junegunn/fzf?style=flat)](https://github.com/junegunn/fzf/stargazers)

---
`![fzf merch](https://junegunn.github.io/fzf/images/fzf-mugs.jpg)
...
| MSYS2 (pacman) | `pacman -S $MINGW\_PACKAGE\_PREFIX-fzf` |
...
>
> \* bash: `FZF\_CTRL\_R\_COMMAND= FZF\_ALT\_C\_COMMAND= eval "$(fzf --bash)"`
> \* zsh: `FZF\_CTRL\_R\_COMMAND= FZF\_ALT\_C\_COMMAND= source <(fzf --zsh)`
> \* fish: `fzf --fish | FZF\_CTRL\_R\_COMMAND= FZF\_ALT\_C\_COMMAND= source`
> \* nushell: add to your `env.nu`:
> `$env.FZF\_CTRL\_R\_COMMAND = ""; $env.FZF\_ALT\_C\_COMMAND = ""`
>
...
> \* Either by setting `$FZF\_DEFAULT\_COMMAND` to a command that generates the desired list
> \* Or by setting `--walker`, `--walker-root`, and `--walker-skip` options in `$FZF\_DEFAULT\_OPTS`
> [!WARNING]
...
> [!TIP]
> You can add these options to `$FZF\_DEFAULT\_OPTS` so that they're applied by
> default. For example,
>
> ```sh
> # Open in a popup if on tmux or Zellij, otherwise use --height mode
> export FZF\_DEFAULT\_OPTS='--height 40% --popup bottom,40% --layout reverse --border top'
> ```
### Search syntax
...
- e.g. `export FZF\_DEFAULT\_COMMAND='fd --type f'`
- `FZF\_DEFAULT\_OPTS`
- Default options
- e.g. `export FZF\_DEFAULT\_OPTS="--layout=reverse --inline-info"`
- `FZF\_DEFAULT\_OPTS\_FILE`
- If you prefer to manage default options in a file, set this variable to
point to the location of the file
- e.g. `export FZF\_DEFAULT\_OPTS\_FILE=~/.fzfrc`
> [!WARNING]
> `FZF\_DEFAULT\_COMMAND` is not used by shell integration due to the
> slight difference in requirements.
>
> \* `CTRL-T` runs `$FZF\_CTRL\_T\_COMMAND` to get a list of files and directories
> \* `ALT-C` runs `$FZF\_ALT\_C\_COMMAND` to get a list of directories
> \* `vim ~/\*\*` runs `fzf\_compgen\_path()` with the prefix (`~/`) as the first argument
> \* `cd foo\*\*` runs `fzf\_compgen\_dir()` with the prefix (`foo`) as the first argument
>
...
![](https://raw.githubusercontent.com/junegunn/i/master/fzf-4-borders.png)
...
[![](https://i.imgur.com/vtG8olE.png)](https://www.youtube.com/watch?v=qgG5Jhi_Els "fzf - command-line fuzzy finder")
Examples
--------
...
- You can override the behavior by setting `FZF\_CTRL\_T\_COMMAND` to a custom command that generates the desired list
- Or you can set `--walker\*` options in `FZF\_CTRL\_T\_OPTS`
- Set `FZF\_CTRL\_T\_OPTS` to pass additional options to fzf
```sh
...
```
- Can be disabled by setting `FZF\_CTRL\_T\_COMMAND` to an empty string when
sourcing the script
...
- Set `FZF\_CTRL\_R\_OPTS` to pass additional options to fzf
```sh
...
```
- Can be disabled by setting `FZF\_CTRL\_R\_COMMAND` to an empty string when
sourcing the script
- Custom override via a non-empty `FZF\_CTRL\_R\_COMMAND` is not yet supported and will emit a warning
- `ALT-C` - cd into the selected directory
- The list is generated using `--walker dir,follow,hidden` option
- Set `FZF\_ALT\_C\_COMMAND` to override the default command
- Or you can set `--walker-\*` options in `FZF\_ALT\_C\_OPTS`
- Set `FZF\_ALT\_C\_OPTS` to pass additional options to fzf
```sh
...
--preview 'tree -C {}'"
```
- Can be disabled by setting `FZF\_ALT\_C\_COMMAND` to an empty string when
sourcing the script
Display modes for these bindings can be separately configured via
`FZF\_{CTRL\_T,CTRL\_R,ALT\_C}\_OPTS` or globally via `FZF\_DEFAULT\_OPTS`.
(e.g. `FZF\_CTRL\_R\_OPTS='--popup bottom,60% --height 60% --border top'`)
...
- `COMMAND [DIRECTORY/][FUZZY\_PATTERN]\*\*`
```sh
...
commands as well by using `\_fzf\_setup\_completion` helper function.
```sh
...
For a command named \_"COMMAND"\_, define `\_fzf\_complete\_COMMAND` function using
`\_fzf\_complete` helper.
```sh
...
`\_fzf\_complete\_COMMAND\_post` as follows.
```sh
\_fzf\_complete\_foo() {
...
- The only supported configuration variables are `FZF\_COMPLETION\_OPTS` and
`FZF\_EXPANSION\_OPS`.
- The function that is used by custom completion functions is named
`fzf\_complete`, which only accepts fzf options as arguments, and can be also
...
a function named `\_fzf\_complete` is provided, that can accept ` -- $argv` in
...
command line, by defining a function named `\_fzf\_complete\_COMMAND\_post` or
`\_fzf\_post\_complete\_COMMAND`:
```fish
...
[external completer](https://www.nushell.sh/cookbook/external\_completers.html)
...
built-in commands (e.g. `cd`, `ls`). Fuzzy completion with `\*\*` only
works for external commands.
- Custom completers can be defined via the `$env.FZF\_COMPLETERS` record in
...
`FZF\_COMPLETION\_TRIGGER`, `FZF\_COMPLETION\_OPTS`,
`FZF\_COMPLETION\_PATH\_OPTS`, `FZF\_COMPLETION\_DIR\_OPTS`,
`FZF\_COMPLETION\_DIR\_COMMANDS`.
Vim plugin
----------
...
> not a good idea to add `--preview` option to your `$FZF\_DEFAULT\_OPTS`\*\*.
>
> ```sh
> # \*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*
> # \*\* DO NOT DO THIS! \*\*
...
searcher](https://github.com/ggreer/the\_silver\_searcher) to traverse the file
...
When using a custom `FZF\_CTRL\_T\_COMMAND`, use the unexpanded `$dir` variable to
